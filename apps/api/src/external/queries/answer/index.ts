...
export class Query implements AnswerQuery.Query {
  ...
  async findOne(by: { answerId: string; teamId: string; userId: string }) {
    const [row] = await db
      .select(
        "users_on_tasks.*",
        "tasks.title as task_title",
      ...
      .andWhere("users_on_tasks.id", by.answerId)
      .orderBy("users_on_tasks.created_at", "desc")
      .innerJoin("tasks", (query) => {
        query
          .on("tasks.id", "=", "users_on_tasks.task_id")
